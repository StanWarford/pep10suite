...
void handle_asm(command_line_values&, QSharedPointer<MacroRegistry>, QRunnable**);
...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setApplicationVersion("10.0.0");
   ...
   parameter_formatting.insert_or_assign("run", std::map<std::string,std::string>());
    ...
    // Subcommands for MACROS
    auto macros_subcommand = parser.add_subcommand("macros", macros_description);
    macros_subcommand->callback(std::function<void()>([&](){handle_macros(values, registry);}));
...
void handle_asm(command_line_values &values, QSharedPointer<MacroRegistry> registry,
                QRunnable **runnable)
{
...
void handle_macros(command_line_values &values, QSharedPointer<MacroRegistry> registry)
{
